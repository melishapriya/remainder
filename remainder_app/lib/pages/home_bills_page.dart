...
class HomeBillsPage extends StatefulWidget {
  const HomeBillsPage({super.key});

@override
  State<HomeBillsPage> createState() => _BankBillsPageState();
}

class _BankBillsPageState extends State<HomeBillsPage> with SingleTickerProviderStateMixin {
  ...
  final List<Map<String, dynamic>> bankbillData = [
    {
  ...
  Widget buildInsuranceList(String status) {
    final filteredData = bankbillData.where((item) => item['status'] == status).toList();
